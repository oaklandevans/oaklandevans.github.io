...
interface Character {
  char: string
  status: 'cursor' | 'done' | 'pending'
}

function KeyType(): ReactElement {
  ...
  const startGame = (): void => {
    ...
    const chars: Character[] = text.split('').map((char, index) => ({
      char,
    ...
    const keydown = (event: KeyboardEvent): void => {
      ...
      if (key === currentChar.char || (key === 'Enter' && currentChar.char === '\n')) {
        ...
        if (nextIndex < newChars.length) {
          ...
          const delta = endTime.getTime() - (startTimeRef.current?.getTime() ?? 0)
